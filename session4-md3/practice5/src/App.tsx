...
export default function App() {
  const [submissionResult, setSubmissionResult] = useState<CreditCardFormValues | null>(null)
  ...
  const creditCardSchema = Yup.object().shape({
    ...
    monthlyIncome: Yup.number()
      .typeError('Thu nhập hàng tháng phải là một số hợp lệ (không chứa chữ hoặc ký tự đặc biệt)')
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-slate-900/60 backdrop-blur-md p-6 rounded-2xl border border-slate-800/80">
          <div>
            ...
            <p className="text-slate-400 text-sm mt-1 max-w-xl">
              Apply for a credit card securely. All information is validated in real-time on the client-side using Formik state management and Yup schema validation.
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-8 items-start">
            ...
            {submissionResult ? (
              ...
              <div className="p-6 bg-emerald-950/20 border border-emerald-500/25 rounded-2xl space-y-4 animate-fade-in">
                <div className="flex items-center gap-3 text-emerald-400">
                  ...
                  <div>
                    <h3 className="font-bold text-base font-outfit">Hồ Sơ Đã Gửi Thành Công!</h3>
                    <p className="text-xs text-slate-400">Yêu cầu mở thẻ tín dụng của bạn đang được duyệt.</p>
                  </div>
                </div>
                ...
                <div className="mt-4 p-4 rounded-xl bg-slate-950/80 border border-slate-850 space-y-3 text-xs font-mono">
                  <div className="flex justify-between border-b border-slate-900 pb-2">
                    ...
                    <span className="text-slate-200 font-bold">{submissionResult.fullName}</span>
                  </div>
                  <div className="flex justify-between border-b border-slate-900 pb-2">
                    <span className="text-slate-500">Số CCCD:</span>
                    <span className="text-slate-200">{submissionResult.cccd}</span>
                  </div>
                  <div className="flex justify-between pb-1">
                    <span className="text-slate-500">Thu nhập tháng:</span>
                    <span className="text-emerald-400 font-bold">
                      {parseInt(submissionResult.monthlyIncome, 10).toLocaleString('vi-VN')} đ
                    </span>
                  </div>
                </div>
              ...
              <form onSubmit={formik.handleSubmit} className="space-y-5">
                ...
                {/* CCCD */}
                <div className="flex flex-col gap-2">
                  <label className="text-xs font-semibold text-slate-400 uppercase">Căn cước công dân (CCCD - 12 số)</label>
                  <div className="relative">
                ...
                {/* Monthly Income */}
                <div className="flex flex-col gap-2">
                  <label className="text-xs font-semibold text-slate-400 uppercase">Thu Nhập Hàng Tháng (đ)</label>
                  <div className="relative">
                  ...
                  {formik.touched.monthlyIncome && formik.errors.monthlyIncome && (
                    <div className="text-xs text-rose-400 font-semibold flex items-center gap-1.5 mt-1">
                      <i className="fa-solid fa-circle-exclamation text-[10px]"></i>
                      <span>{formik.errors.monthlyIncome}</span>
                    </div>
                  )}
                </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 flex flex-col justify-between">
              <div>
                <span className="text-xs font-semibold text-indigo-400 uppercase tracking-wider">Formik & Yup Live State HUD</span>
                <div className="mt-4 space-y-3 text-xs font-mono bg-slate-950 p-4 rounded-xl border border-slate-855">
                  ...
                  <div>
                    <span className="text-slate-550">fullName:</span>{' '}
                    <span className="text-slate-200">"{formik.values.fullName}"</span>
                  </div>
                  <div>
                    <span className="text-slate-555">cccd:</span>{' '}
                    <span className="text-slate-200">"{formik.values.cccd}"</span>
                  </div>
                  <div>
                    <span className="text-slate-555">monthlyIncome:</span>{' '}
                    <span className="text-slate-200">"{formik.values.monthlyIncome}"</span>
                  </div>

                  <p className="text-amber-500 font-bold border-b border-slate-850 pb-1 pt-2">VALIDATION ERRORS</p>
                  <div>
                    <span className="text-slate-550">fullName:</span>{' '}
                    <span className={formik.errors.fullName ? 'text-rose-400' : 'text-slate-550'}>
                      {formik.errors.fullName ? `"${formik.errors.fullName}"` : 'None'}
                    </span>
                  </div>
                  <div>
                    <span className="text-slate-550">cccd:</span>{' '}
                    <span className={formik.errors.cccd ? 'text-rose-400' : 'text-slate-550'}>
                      {formik.errors.cccd ? `"${formik.errors.cccd}"` : 'None'}
                    </span>
                  </div>
                  <div>
                    <span className="text-slate-550">monthlyIncome:</span>{' '}
                    <span className={formik.errors.monthlyIncome ? 'text-rose-400' : 'text-slate-550'}>
                      {formik.errors.monthlyIncome ? `"${formik.errors.monthlyIncome}"` : 'None'}
                    </span>
                  </div>
                </div>
              </div>
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 space-y-3">
              ...
              <ul className="text-xs text-slate-400 space-y-2">
                <li className="flex items-start gap-2">
                  ...
                  <span>Họ và tên: Bắt buộc, không chứa số, tối thiểu 2 ký tự.</span>
                </li>
                <li className="flex items-start gap-2">
                  <i className="fa-solid fa-circle-info text-blue-450 mt-0.5"></i>
                  <span>CCCD: Phải chứa chính xác 12 chữ số (Regex: <code className="text-slate-300">/^\d{12}$/</code>).</span>
                </li>
                <li className="flex items-start gap-2">
                  <i className="fa-solid fa-circle-info text-blue-450 mt-0.5"></i>
                  <span>Thu nhập: Phải là số thực, lớn hơn 5.000.000đ (đầu vào chữ báo lỗi định dạng số).</span>
                </li>
              </ul>
            </div>
          </div>
